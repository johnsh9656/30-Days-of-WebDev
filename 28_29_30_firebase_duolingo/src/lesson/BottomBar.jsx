...
function BottomBar({ checkAvailable, handleCheck, answerState, continueLesson, skip, gameOver, retry, halfway, exitLesson }) {
    ...
    if (gameOver.status) {
        ...
        const exitLessonCallback = gameOver.outcome == 
            "complete" ? exitLesson.handleComplete : exitLesson.handleFail;
        
        return (<div className={footerClass}>
            <div className={containerClass}>
                <h2>{stateImg}</h2>
                <button 
                    onClick={exitLessonCallback}
                >Finish</button>
            </div>
        </div>)
    }
